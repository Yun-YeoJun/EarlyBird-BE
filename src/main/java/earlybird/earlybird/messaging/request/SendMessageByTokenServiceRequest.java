...
public class SendMessageByTokenServiceRequest {
  ...
  @Builder
  private SendMessageByTokenServiceRequest(
      String title, String body, String deviceToken, Long notificationId) {
    this.title = title;
    this.body = body;
  ...
  public static SendMessageByTokenServiceRequest from(
    ...
    String title =
        (notificationStep.equals(ONE_HOUR_BEFORE_PREPARATION_TIME))
            ? appointmentName + notificationStep.getTitle()
            : notificationStep.getTitle();
