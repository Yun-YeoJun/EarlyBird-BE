...
public class NotificationInfoFactory {
    @Getter
    private final List<NotificationStep> notificationStepList;

    public NotificationInfoFactory() {
        this.notificationStepList = List.of(
    ...
    public Map<NotificationStep, Instant> createTargetTimeMap(
            Instant preparationTimeInstant, Instant movingTimeInstant, Instant appointmentTimeInstant
    ) {
        return Map.of(
                ONE_HOUR_BEFORE_PREPARATION_TIME, preparationTimeInstant.minus(1, ChronoUnit.HOURS),
                FIVE_MINUTES_BEFORE_PREPARATION_TIME, preparationTimeInstant.minus(5, ChronoUnit.MINUTES),
                ...
                FIVE_MINUTES_BEFORE_APPOINTMENT_TIME, appointmentTimeInstant.minus(5, ChronoUnit.MINUTES),
                APPOINTMENT_TIME, appointmentTimeInstant
        );
    }

    public Map<NotificationStep, Instant> createTargetTimeMap(
            LocalDateTime preparationTime, LocalDateTime movingTime, LocalDateTime appointmentTime
    ) {
        ...
        return this.createTargetTimeMap(preparationTimeInstant, movingTimeInstant, appointmentTimeInstant);
    }
}
