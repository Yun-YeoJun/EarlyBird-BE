...
class RegisterOneNotificationAtSchedulerServiceTest {
  ...
  void register() {
    // given
    RegisterOneNotificationAtSchedulerService service =
        new RegisterOneNotificationAtSchedulerService(fcmNotificationRepository, schedulerManager);

    Appointment appointment = CreateTestAppointment.create();

    Map<NotificationStep, Instant> notificationInfo =
        Map.of(APPOINTMENT_TIME, Instant.now().plusSeconds(10));
  ...
  void test() {
    ...
    Appointment appointment = CreateTestAppointment.create();
    Map<NotificationStep, Instant> notificationInfo =
        Map.of(APPOINTMENT_TIME, Instant.now().minusSeconds(1));
